...
const NEWS_API_KEY = process.env.NEWS_API_KEY || '226f4b0b49184c869c57081b9cefff43';
const GNEWS_API_KEY = process.env.GNEWS_API_KEY || '4d2e7d3a50fbf5e30ab2d69b92c37e32';
const MEDIASTACK_API_KEY = process.env.MEDIASTACK_API_KEY || 'ac4fa17fc99e68dc90fa3f66175fa70c';

// Additional API keys for new services
const WORLD_NEWS_API_KEY = process.env.WORLD_NEWS_API_KEY || 'YOUR_WORLD_NEWS_API_KEY';
const NEWSAPIAI_API_KEY = process.env.NEWSAPIAI_API_KEY || 'YOUR_NEWSAPIAI_KEY';
...
async function getTopHeadlines(params = {}) {
    try {
        ...
        if (response.articles && response.articles.length > 0) {
            ...
            console.log('No headlines found through NewsAPI, trying backup API...');
    ...
    try {
        ...
        const [recentNews, historicalNews] = await Promise.all([
            ...
            Promise.all(enhancedQuery.queryVariations.flatMap(queryVariation => [
                ...
                newsapi.v2.everything({
                    q: queryVariation,
                    from: enhancedQuery.timeRanges[0].from,
                    to: enhancedQuery.timeRanges[0].to,
                    language: 'en',
                    sortBy: 'publishedAt',
                    pageSize: 100
                })
